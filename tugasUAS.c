...
void inputData(struct Pelajar *pelajar) {
    ...
    printf("4. Lainnya\n");
    printf("Pilih : ");
    scanf("%d", &pilihJurusan);
    
    switch (pilihJurusan) {
        case 1:
            strcpy(pelajar->jurusan, "IPA");
            break;
        case 2:
            strcpy(pelajar->jurusan, "IPS");
            break;
        case 3:
            strcpy(pelajar->jurusan, "Bahasa");
            break;
        default:
            strcpy(pelajar->jurusan, "Lainnya");
            break;
    }
...
void outputData(struct Pelajar *pelajar) {
    ...
    printf("Jenis Kelamin : %s\n", 
        pelajar->jenis_kelamin.laki_laki[0] ? pelajar->jenis_kelamin.laki_laki : pelajar->jenis_kelamin.perempuan);
...
int main() {
    ...
    do {
        ...
        if (pilihan == 1) {
            if (jumlahPelajar == 0) {
                pelajarArray = (struct Pelajar*)malloc(sizeof(struct Pelajar));
                if ( pelajarArray == NULL) {
                    printf("Error: Tidak ada memori yang tersedia.\n");
                    return 1;
                }
